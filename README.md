...
The aim of this article is to show how to improve the capability of
the nmap network scanner to detect SAP services. This is by no mean a
complete and 100% exact way of doing service detection as a lot of
corner cases exists that are not covered in this text. If you want a
more comprehensive way to do SAP services detection and much more, the
ERPScan Monitoring Suite is a good start with its port scanner
feature.

## How nmap can help us

Our goal is to detect each network service exposed by SAP
servers. Those sevrers are complex beasts with a lot of components by
default exposed to the network and each one of these components has
potential vulnerabilities. So we want to send specific network probes
to detect presence of each of those services and then better assess if
a service is potentially vulnerable or not.

nmap is an open source network port scanner that can do a lot of
things and especially service detection via fingerprints. We will
explain how one could implement a SAP-aware port scanner with this
tool.

## SAP support in official nmap

First of all, if you look closely at official nmap release you will
...
If we look at columns SERVICE and VERSION we see a lot of unknown or
not properly named fields. We want to improve this situation by
analyzing each unknown port/protocol.
...
The format is quite self-explanatory for its main features. Let's
...
the `Probe` line describe the TCP payload that we send to the
server. In this case we connect to the TCP port without sending any
TCP payload after the 3-way handshake.

The next line beginning with `match` describe what we want to check
from the server's answer. A match is final, the parser won't check
other match for the given probe (as long as we don't use
...
That probe can be used many time for all those protocols that are
based on a first message sent by the server to the client (SSH, FTP,
mail protocols for exmaple). We just add others `match` line after the
`Probe`.
...
So now we need a way to know which packets are to be sent and what
know specific piece of information inside the answer that can allow us
to identify with a good assurance what protocol is being used and from
...
On a side note SAP services implement a lot of different binary
protocols that doesn't ease our task.
...
hostname. That's what we accomplish by using regular expression groups
(using parenthesis inside the expression) and by referencing them via
their position in the `v//` and `h//` statement (`v` standing for
version, and `h` for hostname).


## How to handle scan port range

Usually with nmap if we don't specify `-p` option it will scan the
1000 most used port (from internet statistics). Unfortunately a lot of
the SAP ports will be missed by doing so. Either we need to scan all
65535 ports to not miss anyone, at a big scan time cost or we look a
bit closer at how to generate those SAP ports. For efficiency reason
we decide to choose the second option.

By looking at SAP documentation we see their have rules to define
potential ports for each services. So by using those rules we can
...
There is no generic answer to this problem if we don't want to scan
...
upon the SSL layer. In practice those specified ports were not
...
Code exploits / port information disclosure in NSA Lua scripts tagged
by categories:
...
If you're a pentester you probably have a bag full of exploits for
specific SAP services, so you want to automatically link open ports to
exploits attempts. That can be easily done by storing the nmap scan
into an XML file (`-oX` option) and then writing a parser that will
generate exploit command-line to be executed on the specific open
ports.

On ther other hand if you are a security analyst or doing operational
...
to pinpoint vulnerable services by their version. For this mean we use
the [IVRE framework](https://ivre.rocks) that can import our XML nmap
scans and provides a nice web interface to query scan results and
allows to do basic statistics/reporting tasks.

Attached screenshots shows a scan in IVRE with filtering OFFICE
...
level probes we can go deep in the knowledge of what's behind a SAP
server.

In the meantime this blog post is a way to remind that SAP servers
have a huge exposition surface and all their service ports shouldn't
be available to the users. Enforcing a strict networking policy is
part of a good security hygiene.
